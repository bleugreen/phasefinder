...
def train_one_epoch(epoch: int) -> float:
    ...
    for i, (stft, beat_phase) in enumerate(tqdm(train_loader)):
        ...
        stft = stft.to(device)
        beat_phase = beat_phase.to(device)
        ...
        if (i + 1) % 1000 == 0:
            writer.add_scalar("Loss/Train", window_loss / 1000, epoch * len(train_loader) + i)
            window_loss = 0.0
...
def test(epoch: int) -> tuple[float, float, float]:
    overall_f_measure, overall_cmlt, overall_amlt = test_model_f_measure(model, args.data_path, device=device)
